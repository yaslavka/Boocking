          ...
          <Row className={styles.row}>
            <Col className={styles.col1}>
              <article className={styles.numberImages}>
                {number?.imageNumber && (
                  <img
                    src={number?.imageNumber}
                    alt={number.imageNumber}
                    width={200}
                    height={'auto'}
                  />
                )}
                <div className={styles.nameNumber}>
                  {number?.nameNumber}
                  <div className={styles.typeNumber}>{number?.typeNumber}</div>
                </div>
              </article>
              <article className={styles.description}>
                <span className={styles.textDescription}>
                  Количество комнат:{' '}
                  <span className={styles.text}>{number?.count}</span>
                </span>
                <span className={styles.textDescription}>
                  Квадратура:{' '}
                  <span className={styles.text}>{number?.quadrature}</span>
                </span>
                <span className={styles.textDescription}>
                  Кровати:{' '}
                  <span className={styles.text}>{number?.sleepingPlaces}</span>
                </span>
                <span className={styles.textDescription}>
                  Гостей:{' '}
            ...
            <Col className={styles.col2}>
              <article className={styles.service}>
                ...
                <div className={styles.serviceText}>Полный пансион влючен</div>
              </article>
              <article className={styles.service}>
                <img src={bank} alt={bank} />
                <div className={styles.serviceText}>Безналичная оплата</div>
              </article>
              <Button
                color="primary"
                role={'link'}
                href={`/number/${number?.id}`}
                className={styles.margiB}
              >
                Обзор номера
              </Button>
