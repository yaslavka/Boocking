...
function MyNumbersEdit() {
  ...
  useEffect(() => {
    if (imageHotel !== null) {
      dispatch(
        numberIdActions.uploadImages({ imageNumber: imageHotel, id: id }),
      )
    }
    ...
    (credentials) => {
      dispatch(
        numberIdActions.numberIdEdit({
          ...
          descriptionNumber:
            credentials.descriptionNumber || numberId?.descriptionNumber,
          typeNumber: credentials.typeNumber || numberId?.typeNumber,
          sleepingPlaces:
            credentials.sleepingPlaces || numberId?.sleepingPlaces,
  ...
  return (
    <PrivateNavbar>
      {numberId && (
        <>
          <h5 className={styles.title}>Редактирование {numberId.nameNumber}</h5>
