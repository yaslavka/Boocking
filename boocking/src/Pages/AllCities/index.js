...
function AllCities() {
  ...
  const currentPages = allCities && allCities.slice(firstTaskIndex, lastTaskIndex);
          ...
          <>
            ...
            <section className={styles.pagesButtonContainer}>
              {pagesNumber.map((numberPages)=>(
                <button key={numberPages} onClick={()=>{
                  nextPages(numberPages);
                }} className={styles.pagesButton}>{numberPages}</button>
              ))}
            </section>
          </>
        )}
