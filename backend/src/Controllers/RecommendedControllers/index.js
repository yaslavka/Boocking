...
class RecommendedControllers{
    async recommended(req, res){
        const recommended = await HotelModals.findAll({where: {bal: 10}, include:[{model: ReviewModels, as: 'review'}]})
    ...
    async actions(req, res){
        const actions = await HotelModals.findAndCountAll({discount: {[Op.gt]: 0}, include:[{model: ReviewModels, as: 'review'}]})
